(function() {
  ...
  var tmpl = [
    ...
    '<div class="angularjs-datetime-inner">',
    '  <div class="adp-month">',
    ...
    '  </div>',
    '  <div class="adp-days">',
    '    <div class="adp-day-of-week" ><div ng-repeat="dayOfWeek in ::daysOfWeek" title="{{dayOfWeek.fullName}}">{{::dayOfWeek.firstLetter}}</div></div>',
    '    <div class="adp-day-of-day" ng-click="setDate($event)"><div class="adp-day leadingDays" today="{{today}}" d2="{{mv.year + \'-\' + (mv.month) + \'-\' + day}}" ng-show="mv.leadingDays.length < 7" ng-repeat="day in mv.leadingDays">{{::day}}</div>',
    ...
    '      {{::day}}',
    '    </div>',
    '    <div class="adp-day leadingDays" d2="{{mv.year + \'-\' + (mv.month + 2) + \'-\' + day}}"  ng-show="mv.trailingDays.length < 7" ng-repeat="day in mv.trailingDays">{{::day}}</div>',
    '  </div>',
    '</div>',
    '</div>',
    '</div>'].join("\n");
    ...
    var linkFunc = function(scope, element, attrs, ctrl) { //jshint ignore:line
      ...
      scope.$applyAsync( function() {
        ...
        if (!scope.selectedDate || isNaN(scope.selectedDate.getTime())) { // no predefined date
          ...
          scope.selectedDate = new Date(year, month, day, 0);
        }
        ...
        scope.today = dateFilter(new Date(), 'yyyy-MM-dd');
      ...
      scope.setDate = function (evt) {
        ...
        var clickMonth = target.getAttribute('d2');
        var newMonth = clickMonth.substr(5,1);
        if (target.className.indexOf('selectable') !== -1) {
          scope.updateNgModel(parseInt(target.innerHTML),newMonth);
      ...
      scope.updateNgModel = function(day,month) {
        day = day ? day : scope.selectedDate.getDate();
        scope.selectedDate = new Date(
          scope.mv.year, month - 1, day, 0
        );
        scope.selectedDay = scope.selectedDate.getDate();
        if (attrs.ngModel) {
